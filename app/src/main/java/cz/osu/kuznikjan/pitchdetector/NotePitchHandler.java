...
public class NotePitchHandler {
    ...
    private static String[] noteNames = {"C", "C#", "D", "Eb", "E", "F", "F#", "G", "G#", "A", "A#", "B"};
    ...
    public static NoteResult mapPitchDetectionResultToNoteResult(PitchDetectionResult pitchDetectionResult) {
        NoteResult noteResult = new NoteResult();
        double closenessPercent;
        double closenessCents = 0.00f;
        String outputNote;
        ...
        if ((pitchDetectionResult.getPitch() < PITCH_LOW_LIMIT) || (pitchDetectionResult.getPitch() > PITCH_HIGH_LIMIT)) {
            ...
            for (int i = 0; i < notes.length; i++) {
                ...
                if ((pitchDetectionResult.getPitch() > (comparisonOctave[0] - ALLOWABLE_ERROR)) &&
                        (pitchDetectionResult.getPitch() < (comparisonOctave[notes[i].length - 1] + ALLOWABLE_ERROR))) {
        ...
        for (int i = 0; i < octave.length; i++) {
            double diff = Math.abs(pitchDetectionResult.getPitch() - octave[i]);
            if (diff < bestDifference) {
                bestFitNoteIndex = i;
                bestDifference = diff;
                closenessCents = 1200 * Math.log(pitchDetectionResult.getPitch()/octave[i]);
            }
        }

        closenessPercent = (pitchDetectionResult.getPitch() / octave[bestFitNoteIndex]) * 100;
        outputNote = noteNames[bestFitNoteIndex] + Integer.toString(bestFitOctave);

        noteResult.setPitch(pitchDetectionResult.getPitch());
        noteResult.setPitched(pitchDetectionResult.isPitched());
        noteResult.setProbability(pitchDetectionResult.getProbability() * 100);
        noteResult.setNote(outputNote);
        noteResult.setClosenessPercent(closenessPercent);
        noteResult.setClosenessHz(bestDifference);
        noteResult.setClosenessCents(closenessCents);
